...
public class TestingLista {
	...
	public static Lista intercalar(Lista L1, Lista L2) {
		...
		int posicion = 1, iterador = 1;
		int iteradorUno = 1, longitudUno = L1.longitud();
		int iteradorDos = 1, longitudDos = L2.longitud();
		int longitud = longitudUno + longitudDos;

		while (posicion <= longitud / 2 + 1) {
			if (iteradorUno <= longitudUno) {
				listaNueva.insertar(L1.recuperar(posicion), iterador);
				iteradorUno++;
				iterador++;
			}
			if (iteradorDos <= longitudDos) {
				listaNueva.insertar(L2.recuperar(posicion), iterador);
				iteradorDos++;
				iterador++;
			}
			posicion++;
