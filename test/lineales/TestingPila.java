...
public class TestingPila {
	...
	public static boolean esCapicua(Pila pila) {
		...
		if (!pila.esVacia()) {
			...
			// Apila de forma inversa
			while (!pilaClon.esVacia()) {
				pilaAuxiliar.apilar(pilaClon.obtenerTope());
				pilaClon.desapilar();
			}
			System.out.println(pilaAuxiliar.toString());

			pilaClon = pila.clone();

			// Compara los topes de la original y la inversa
			while (!pilaClon.esVacia() && capicua) {
				if (!pilaClon.obtenerTope().equals(pilaAuxiliar.obtenerTope())) {
					capicua = false;
				}
				pilaClon.desapilar();
	...
	public static boolean pilaCapicua(Pila pila) {
		boolean capicua;
		Pila clon = pila.clone();
		int numeroNormal = 0, numeroInvertido = 0, fact = 1;

		while (!clon.esVacia()) {
			numeroNormal = (numeroNormal * 10) + (int) clon.obtenerTope();
			numeroInvertido += fact * (int) clon.obtenerTope();
			fact *= 10;

			clon.desapilar();
		}

		capicua = numeroNormal == numeroInvertido;
